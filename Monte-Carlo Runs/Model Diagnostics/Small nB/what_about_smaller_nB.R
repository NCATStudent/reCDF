## changes --
### 1. added smaller nB option
### 2. added maximum hack (i.e., if >99% not available, steal maximum of B$y)
...
sim.f2 <- function(N,
  ...
  MI_cdf <- function(B, A, p) {
    ...
    mlm_df <- data.frame(
      ...
      rbind(data.frame("probs" = 1, "t" = max(B$y))) %>% # if >99 not available, steal maximum from B
      arrange(probs)
