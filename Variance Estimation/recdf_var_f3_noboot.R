...
samp.f <- function(samp, A, B_MAR_nA, B_MAR_20nA, B_MNAR_1nA, B_MNAR_20nA, r, pop, p, miss, mc_cores = 20, alp = .10, seed = 101) {
  ...
  var_cdf <- function(B, A, p, alp, da_miss, isboot, mc_cores = 15) {
    ...
    quant_est <- function(F_N, nA) {
      ...
      v1 <- nB / (nB - 1) * (1 - (nA / N)) / nA * var(e_ecdf(hatq - lm_pred_A), na.rm = TRUE)
      v2 <- -1 / (nB - 1) * (1 - (nA / N)) / nA * var(e_ecdf(hatq - lm_pred_A), , na.rm = TRUE)
      ...
      v3 <- sum(da_Ghats, na.rm = TRUE) * (1 / (N^2)) * (1 / (nB - 1))
