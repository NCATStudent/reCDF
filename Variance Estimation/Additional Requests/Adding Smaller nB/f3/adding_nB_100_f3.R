...
if (mod == "f3") {
  ...
  stop("You must select one of the following models: (f3, f2, f3, f4)")
}
...
Bs_MAR_1nA <- mclapply(1:nsim, function(seed) {
  B_generator(miss = "MAR", nB = 100, r = r)
}, mc.cores = 10)
...
Bs_MNAR_1nA <- mclapply(1:nsim, function(seed) {
  B_generator(miss = "MNAR", nB = 100, r = r)
}, mc.cores = 10)
# for testing
...
samp.f <- function(samp, A, B_MAR_nA, B_MNAR_1nA, r, pop, p, miss, L = 1500, mc_cores = 15, alp = .10, seed = 101) {
  ...
  ## nB = 100
  set.seed(seed)
  B_list.MAR_1nA <- mc_replicate(L, list(B_perm_MAR_1nA[sample(1:100,
    size = 100,
    replace = TRUE
  ), ]), mc.cores = mc_cores)

  set.seed(seed)
  B_list.MNAR_1nA <- mc_replicate(L, list(B_perm_MNAR_1nA[sample(1:100,
    size = 100,
    replace = TRUE
  ), ]), mc.cores = mc_cores)
  ...
  final_results <- list(
    var_calcs(A_perm, B_perm_MAR_1nA, L, alp, da_miss = "MAR", da_nB = 100),
    var_calcs(A_perm, B_perm_MNAR_1nA, L, alp, da_miss = "MNAR", da_nB = 100)
  ) %>%
    lapply(bind_rows) %>%
    bind_rows()
...
for (i in 1:nsim) {
  ...
  if (i %% 100 == 0) {
    setwd("/Users/jeremyflood/Library/CloudStorage/OneDrive-Personal/Documents/Grad School/2024-2025/Fall 2025/reCDF/reCDF/Variance Estimation/Additional Requests/Adding Smaller nB/f3/Cached Iter Files")
    results[(i - 100):100] %>%
      bind_rows() %>%
      mutate(mod = mod) %>%
      openxlsx::write.xlsx(
...
setwd("/Users/jeremyflood/Library/CloudStorage/OneDrive-Personal/Documents/Grad School/2024-2025/Fall 2025/reCDF/reCDF/Variance Estimation/Additional Requests/Adding Smaller nB/f3/Plots")
